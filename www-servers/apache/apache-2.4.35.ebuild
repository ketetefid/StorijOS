...
src_install() {
	...
	# New apache conf.d file
	doconfd "${FILESDIR}"/apache2
	# main, SC and XSC chains
	insinto /etc/apache2/vhosts.d/
	doins "${FILESDIR}"/hschain.conf
	doins "${FILESDIR}"/siachain.conf
	doins "${FILESDIR}"/default_vhost.include

	dodir /etc/storij
	insinto /etc/storij
	doins "${FILESDIR}"/2fa-secret
	doins "${FILESDIR}"/hashval
    	fowners apache /etc/storij
	fowners apache /etc/storij/2fa-secret
	fowners apache /etc/storij/hashval
}
